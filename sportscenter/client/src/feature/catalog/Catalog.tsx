...
export default function Catalog() {
    ...
    useEffect(() => {
        Promise.all([
            api.Store.list(currentPage, pageSize),
            api.Store.brands(),
            api.Store.types()
    ...
    const loadProducts = (selectedSort, searchKeyword = '') => {
        ...
        //construct the url
        let url = `${api.Store.apiUrl}?sort=${sort}&order=${order}`;
        ...
        if (searchKeyword) {
            ...
            api.Store.search(searchKeyword)
                .then((productsRes) => {
                    setProducts(productsRes.content);
            ...
            api.Store.list(page, size, undefined, undefined, url)
                .then((productsRes) => {
                    setProducts(productsRes.content);
                    setTotaItems(productsRes.totalElements);
                })
